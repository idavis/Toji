...
$files | % { Copy-Item $_.FullName "$buildPath\$(Split-Path $_ -leaf)" -Force }
...
$files | ? { (!(Test-Path("$buildPath\$(Split-Path $_ -leaf)"))) } | % { Copy-Item $_.FullName "$buildPath\$(Split-Path $_ -leaf)" }

# copy the solution level build scripts, ignore if they already exist

$files = Get-ChildItem "$contentPath\commands\*"

$files | ? { (!(Test-Path("$rootPath\$(Split-Path $_ -leaf)"))) } | % { Copy-Item $_.FullName "$rootPath\$(Split-Path $_ -leaf)" }
